...
class ResUNet(nn.Module):
    def __init__(self, in_c, out_c, features=None, bottleneck_feature=None, 
                 ...
                 target_params='large', width_multiplier=None, add_sphere_grid=False):
        """
    ...
    def forward(self, x):
        """
        Expect x shape either (B, H, W, C) or (B, H, W, T, C).
        If H/W are not divisible by 2**num_downsample_layers, pad to avoid stride/concat mismatches.
        """
        x = x.permute(0, 2, 3, 1) # (B, C, x_grid, y_grid) -> (B, x_grid, y_grid, C)
        if self.add_sphere is not None:
            x = self.add_sphere(x)
        orig_H, orig_W = x.shape[1], x.shape[2]
        num_down = len(self.encoder)
        pad_h = (2 ** num_down - orig_H % (2 ** num_down)) % (2 ** num_down)
        pad_w = (2 ** num_down - orig_W % (2 ** num_down)) % (2 ** num_down)

        # absorb time dimension into the channel dimension if present
        if len(x.shape) == 5:
           B, H, W, T, C = x.shape
        ...
        if T is not None:
            out = out.view(B, orig_H, orig_W, -1, self.out_c)
        else:
            out = out.view(B, orig_H, orig_W, self.out_c)
        out = out.permute(0, 3, 1, 2) # (B, x_grid, y_grid, C) -> (B, C, x_grid, y_grid)
        return out
...
if __name__ == "__main__":
    ...
    # x = torch.rand(2, 96, 192, 7)
    # x = torch.rand(1, 128, 128, 3)
    x = torch.rand(1, 48, 96, 7)
