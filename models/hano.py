...
class HANO2d(nn.Module):
    '''
        input shape: [batch, 3, res, res]
        output shape: [batch, 3, res, res]
    '''
    def __init__(self, 
        ...
        self.T_in = T_in
        self.T_out = T_out
        self.res_output = (res_output, res_output) if isinstance(res_output, int) else res_output
        self.resatt = res_att

        self.encoder = PatchEmbed(patch_size=patch_size, 
                                  in_chans=in_dim * T_in + 2,  # +2 for coordinates dimension
                                  embed_dim=self.feature_dim, 
                                  stride=subsample_attn,
                                  patch_padding=patch_padding)

        self.attn = HAttention(in_chans=self.feature_dim, 
                               depths=depths,
                               num_heads=num_heads, 
                               window_size=window_size)

        self.decoder = Decodermap(in_channels=self.feature_dim,
                                  modes=F_modes, 
                                  width=F_width, 
    ...
    def forward(self, x):
        ...
        # Convert from (B, H, W, C) to (B, C, H, W) to match input format
        x = x.reshape(B, self.res_output[0], self.res_output[1], self.T_out, -1)
        return x
...
class PatchEmbed(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, embed_dim=96, norm_layer=nn.LayerNorm, stride=1, patch_padding=1):
        super().__init__()
        patch_size = to_2tuple(patch_size)
        self.in_chans = in_chans
        self.embed_dim = embed_dim
        # Use kernel_size=3, stride=1, padding=1 to get exactly 128x128 output
        # Output size = (128 + 2*1 - 3)/1 + 1 = 128
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=3, stride=stride, padding=patch_padding)
...
class Decodermap(nn.Module):
    def __init__(self, in_channels=64, modes=12, width=64, num_spectral_layers=5, mlp_hidden_dim=128, output_dim=1, activation='gelu', padding=5, resolution=None, init_scale=16, add_pos=False):
        super().__init__()
        ...
        self.pre_conv = nn.Conv2d(in_channels, self.width, kernel_size=1, stride=1)
    ...
    def forward(self, x, pos=None):
        ...
        # match the shape, incase the output of attention does not match the input of decoder
        x = self.pre_conv(x)
...
if __name__ == "__main__":
    ...
    model = HANO2d(
        boundary_condition='dirichlet',
        feature_dim=64,  # feature dim, in order to enhance expressiveness
        T_in=7,
        T_out=1,
        res_output=128,
        in_dim=3,
