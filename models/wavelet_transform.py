...
class WaveletAttentionBlock(nn.Module):
    def __init__(self, wave='haar', dim=64, use_efficient_attention=False, local_attention_size=8, groups=1, **kwargs):
        ...
        self.conv_post =  self.filter = nn.Sequential(
                nn.Conv2d(dim, dim, kernel_size=3, padding=1, stride=1, groups=groups),
                # nn.BatchNorm2d(dim),
            )
...
if __name__ == "__main__":
    ...
    model = MultiscaleWaveletTransformer2DDecoderNoAttention(input_dim=3, output_dim=1, dim=128, use_efficient_attention=True, groups=4)
