...
class MSWT_strided_up_downsampling(PeriodicMSWT2D_Patching):
    """Uses nn.Conv2d(stride=2) and nn.ConvTranspose2d(stride=2) instead of DWT/IDWT."""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # Rebuild enc_layers with Conv2d stride=2 instead of DWT + CircularConv2d
        dims = np.array(kwargs.get('dims', []))
        if len(dims) == 0 and kwargs.get('dim') is not None:
            dim = kwargs['dim']
            dims = np.array([dim // 2, 2 * dim, 4 * dim, 4 * dim])
        wave = kwargs.get('wave', 'haar')
        use_efficient_attention = kwargs.get('use_efficient_attention', False)
        efficient_layers = kwargs.get('efficient_layers', [0, 1])
        local_attention_size = kwargs.get('local_attention_size')

        self.enc_layers = nn.ModuleList([])
        for i in range(self.n_layers):
            dim = int(dims[i])
            efficient_flag = i in efficient_layers and use_efficient_attention
            attn_layer = nn.ModuleList(
                [
                    nn.LayerNorm(dim),
                    PeriodicWaveletAttentionBlock(
            ...
            out_dim = int(dims[i + 1]) if i < self.n_layers - 1 else dim
            down_layer = nn.ModuleList([
        ...
        self.dec_layers = nn.ModuleList([])
        for i in range(self.n_layers):
            dim = int(dims[self.n_layers - i - 1])
            new_dim = int(dims[self.n_layers - i - 2]) if self.n_layers - i - 2 >= 0 else dim
            skip_dim = dim  # x_prev has dim channels (from encoder at same resolution)
            efficient_flag = self.n_layers - i - 1 in efficient_layers and use_efficient_attention
            up_layer = nn.ModuleList(
                [
                    nn.ConvTranspose2d(dim, new_dim, kernel_size=3, stride=2, padding=1),
                    nn.Conv2d(new_dim + skip_dim, new_dim, kernel_size=3, padding=1),
                ]
            )
            attn_layer = nn.ModuleList(
                [
    ...
    def up_block(self, x, x_prev, layer, h, w):
        ...
        x = rearrange(x, "b (h w) c -> b c h w", h=h, w=w)
        x = conv_t(x)
        # Crop or interpolate to match x_prev resolution
        h_prev, w_prev = x_prev.shape[-2], x_prev.shape[-1]
        if x.shape[-2] != h_prev or x.shape[-1] != w_prev:
            if x.shape[-2] >= h_prev and x.shape[-1] >= w_prev:
                x = x[..., :h_prev, :w_prev]
            else:
                x = F.interpolate(x, size=(h_prev, w_prev), mode="bilinear", align_corners=False)
        x = torch.cat((x, x_prev), dim=1)
        x = conv(x)
        h, w = x.shape[-2], x.shape[-1]
        x = rearrange(x, "b c h w -> b (h w) c")
...
class MSWT_no_attention(PeriodicMSWT2D_Patching):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
...
class MSWT_no_tokenizer(PeriodicMSWT2D_Patching):
    def __init__(self, **kwargs):
        kwargs['patch_size'] = 1
        super().__init__(**kwargs)
...
if __name__ == "__main__":
    ...
    model = MSWT_strided_up_downsampling(input_dim=3, output_dim=1, dims=[64, 64, 128, 512], 
    use_efficient_attention=True,   efficient_layers=[0, 1, 2], add_periodic_grid=True, 
    local_attention_size=8, 
    patch_size=2)
