...
def load_data_model(just_load_path=False):
    ...
    if args.model == "PDERefiner":
        model =  PDERefiner(
                ...
                trajlen=test_dataset[0][1].shape[-2] + args.T_ar, # T_max
                activation='gelu',
                criterion='mse',
            ).to(device)
    else:
...
def predict_and_save(model, test_loader, save=False, log_path=None):
    ...
    with torch.no_grad():
        ...
        for xx, yy in test_loader:
            ...
            xx_norm = test_dataset.normalize_x(xx)
            yy_norm = test_dataset.normalize_x(yy)
                
            xx_norm = xx_norm.permute(0, 3, 4, 1, 2).contiguous() # (B, T_in, C, H, W)
            ...
            for t in range(0, yy_norm.shape[-2], args.T_bundle):
                ...
                xx_norm = torch.cat((xx_norm[:, args.T_bundle:], pred_step.permute(0, 3, 4, 1, 2)), dim=1)
