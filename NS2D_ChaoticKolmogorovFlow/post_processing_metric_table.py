...
def plot_error_energy():
    ...
    for step in steps:
        ...
        color_list = ['orange', 'blue', 'green', 'red', 'purple', 'black']
        linestyle_list = ['-.', '--', '-.', ':', '--', '-']
        ax.loglog(k_np, spectral_true, label='Truth', linewidth=2, color=color_list[0], linestyle=linestyle_list[0])
        for i, model_name in enumerate(model_name_list):
            ax.loglog(k_np_dict[model_name], spectral_pred_dict[model_name], label=f'{plot_model_name_list[i]}', linewidth=1 if model_name != 'MSWT' else 2, color=color_list[i+1], linestyle=linestyle_list[i+1])
        ax.set_xlabel('Wavenumber k')
        ax.set_ylabel('Spectral Energy Spectrum E(k)')
        ...
        for i, model_name in enumerate(model_name_list):
            ax.loglog(k_np_dict[model_name], enstropy_pred_dict[model_name], label=f'{plot_model_name_list[i]}', linewidth=1 if model_name != 'MSWT' else 2, color=color_list[i+1], linestyle=linestyle_list[i+1])
        ax.set_xlabel('Wavenumber k')
