...
def preprocess_ns2d():
    ...
    preprocess('/home/haozhongkai/files/ml4phys/mgn/pdessl/data/ns2d_fno/ns2d_1e-5_train.pkl')


def preprocess_mat():
    data = h5py.File('/scratch3/wan410/operator_learning_data/ns2d_fno/ns_V1e-3_N5000_T50.mat')
    data = np.array(data['u'])
    data = np.transpose(data, (3,1,2,0))
    train_u = data[:-200]
    test_u = data[-200:]
    print(train_u.shape, test_u.shape)
    pickle.dump(train_u, open('/scratch3/wan410/operator_learning_data/ns2d_fno/ns2d_1e-3_train.pkl','wb'))
    pickle.dump(test_u, open('/scratch3/wan410/operator_learning_data/ns2d_fno/ns2d_1e-3_test.pkl','wb'))
...
def save_hdf5():
    ...
    direc = '/scratch3/wan410/operator_learning_data/ns2d_fno'
    for fname in file_names:
        if not os.path.exists(os.path.join(direc, fname)):
            print('file %s does not exist'%fname)
            continue

        # 读取pickle文件
        with open(os.path.join(direc,fname), 'rb') as f:
            data = pickle.load(f)
        ...
        # 将数据写入hdf5文件
        with h5py.File(os.path.join(direc,hdf5_name), 'w') as hf:
