...
def autoregressive_predict(model, test_loader, device, grid):
    ...
    with torch.no_grad():
        ...
        total_pred = torch.cat(total_pred, dim=0) # (N, S, S, C, T+1)
        initial_condition = torch.cat(initial_condition, dim=0) # (N, S, S, C)
        total_ground_truth = torch.cat(total_ground_truth, dim=0).permute(0, 1, 2, 4, 3) # (N, S, S, C, T)
